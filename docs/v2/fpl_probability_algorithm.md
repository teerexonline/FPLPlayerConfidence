# FPL Goal & Assist Probability Algorithm — Implementation Spec (v1.3.1)
...
> **NOTE:** This step is amended by `fpl_probability_algorithm_v1.3_patch.md` Gap A. The patch corrects a Poisson interpretation issue and introduces `BASELINE_ATTACKING_EVENTS_PER_MATCH = 12`. **Use the patched version of Step 4, not the version below.** The version below is preserved for reference and changelog continuity.
...
```
# Probability the player is involved in any given attacking event
p_involved = influence_pct

# Conditional probabilities given involvement
p_goal_given_involved   = threat_pct       # already DEFENDER_THREAT_SCALE-adjusted if DEF
p_assist_given_involved = creativity_pct
